...
## Сопоставление с прайсом (S4 polish, 2026-07-22)

Порядок (`mapping/program_price_matcher.py` → `match_program_items_to_price`):

1. **Phrase-rules** — типичные формулировки ПМИ («коэффициент затухания ОК», «осевое кручение»…)  
2. **`test_aliases`** (S5 seed + YAML)  
3. Точное / подстрочное имя прайса  
4. **Token-fuzzy** (Jaccard по токенам, min ~0.42)  
5. `map_requirements_to_tests` (с поправкой: оптическое затухание ≠ экранирование)

Контекст программы (название / ТУ / «оптические») влияет на дизамбигуацию  
(например, огнестойкость → `огнестойкость_оптического_кабеля`).

| Метрика | Значение |
|---------|----------|
| GUI | строка **«сопоставлено N/M (xx%)»** над позициями |
| CLI | `match-program-price --id N` / `--overwrite` / `-v` |
| Референс | ППИ Вулкан (ТУ 27.31.11-131): **baseline ~5/14 → ≥9/14** |

Не всё сопоставится: нет аналога в прайсе (маркировка, срок службы, раздавливание…) — это нормально.  
Допишите alias/маппинг в **Настройки** или `add-test-alias`.
