...
| Логи | если `data\logs` на сети недоступен → `%LOCALAPPDATA%\Lab_request\logs` |

Проверка на work после update: меню → папка логов; файл `app_YYYY-MM-DD.log` за сегодняшний день.
